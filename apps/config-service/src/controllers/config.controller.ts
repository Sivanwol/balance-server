...
export class ConfigController {
  ...
  @OpenAPI({
    summary: 'will toggle what ever platform is on ',
    security: [
      {
        verifyServiceCode: [],
      },
  ...
  @Get('/sync')
  @OpenAPI({
    summary: 'will force request sync request to all micro service maintenance mode',
  })
  ...
  async sync() {
    ...
    try {
      ...
      if (totalActiveJobsCount === 0 || queues[queueKeys.sync].scheduler.isRunning()) {
        ...
        await queues[queueKeys.sync].queue.add(
          queueKeys.sync,
          {},
          {
            attempts: 3,
  ...
  @Get('/sync/service')
  @OpenAPI({
    summary: 'will force request sync request to a service',
    security: [
      {
        verifyServiceCode: [],
      },
  ...
  async allConfig() {
    ...
    try {
      ...
      logger.error(`get all settings end up with error (${e.message}`, {
        error: e,
      });
      delete response.data;
  ...
  async updateSettings(@Param('keyName') keyName: string, @BodyParam('value', { required: true }) value: string, @Req() request: any) {
    ...
    try {
      ...
      logger.error(`Update setting end up with error (${e.message}`, {
        error: e,
      });
      response.errorCode = 500;
